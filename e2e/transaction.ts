...
export async function declareContract(
  ...
  const pt = path.resolve(
    __dirname,
    `../contracts/target/dev/rosettacontracts_${contractName}.contract_class.json`,
  )
  const compiledCasm = path.resolve(
    __dirname,
    `../contracts/target/dev/rosettacontracts_${contractName}.compiled_contract_class.json`,
  )
