...
_Additional extensions will be developed to provide integrability for existing Starknet protocols._
...
To build and start the devnet, run the following command:
...
To stop the running devnet, execute this command:

```bash
yarn stop:devnet
```

#### Starting the Stopped Devnet

If the devnet has been stopped and you want to start it again, use the command:

```bash
yarn start:devnet
```

#### Starting the Devnet with Node and Console Sniffing

To start both the devnet and node (with console sniffing enabled), run:

```bash
yarn start:dev+devnet:console-sniff
```
