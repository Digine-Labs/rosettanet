...
import {
  callStarknetEstimateFee,
  getStarknetAccountNonce,
...
export async function estimateGasHandlerx(
  ...
  const snToAddress: string | StarknetRPCError =
  ...
  const snFromAddress: string | StarknetRPCError =
    await getSnAddressFromEthAddress(from)
  ...
  const accountNonce: string | StarknetRPCError =
    await getStarknetAccountNonce(snFromAddress)
  ...
  const actualValue =
    parameters.value == null ? BigInt(0) : BigInt(parameters.value)
  ...
  if (
    ...
    const estimateFeeTransaction: EstimateFeeTransaction = {
      ...
      maxGasPricePerUnit:
        parameters.gasPrice == null ? '0x0' : parameters.gasPrice,
      signature,
      calldata: [],
      directives: [],
      nonce: accountNonce,
    ...
    const rosettanetCalldata = prepareRosettanetCalldataForEstimatingFee(
      estimateFeeTransaction,
    )

    //const rosettanetCalldata = prepareRosettanetCalldataForEstimateTransaction(parameters, [], []);

    const estimatedFee: RPCResponse | StarknetRPCError =
      await callStarknetEstimateFee(
        snFromAddress,
        estimateFeeTransaction,
        rosettanetCalldata,
      )
    ...
    if (
      typeof result.gas_consumed === 'undefined' ||
      typeof result.gas_price === 'undefined' ||
      typeof result.overall_fee === 'undefined'
    ) {
      return <RPCError>{
        jsonrpc: request.jsonrpc,
        id: request.id,
  ...
  const targetContract: StarknetContract | StarknetContractReadError =
    await getContractAbiAndMethods(snToAddress)
  ...
  const estimateFeeTransaction: EstimateFeeTransaction = {
    ...
    directives: EVMCalldataDecode.directives,
    nonce: accountNonce,
    value: actualValue,
    targetFunction: starknetFunction,
  }
  ...
  const totalFee = addHexPrefix(
    new BigNumber(result.gas_consumed).plus(5000).toString(16),
  )
  return <RPCResponse>{
    jsonrpc: request.jsonrpc,
    id: request.id,
