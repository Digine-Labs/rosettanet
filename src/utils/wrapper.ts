...
const SELECTORS = {
  ...
  get_sn_address_with_fallback: '0x0137486de5a7278d659b176b17233ababeb35d63c9c605c7da04d8a7f1978067'
}
...
export async function getSnAddressWithFallback(ethAddress: string): Promise<string | StarknetRPCError> {
  const rosettanetContract = getConfigurationProperty('rosettanet');
  const request: RPCRequest = {
    jsonrpc: '2.0',
    method: 'starknet_call',
...
export async function precalculateStarknetAccountAddress(ethAddress: string): Promise<string | StarknetRPCError> {
  const rosettanetContract = getConfigurationProperty('rosettanet');
  const request: RPCRequest = {
    jsonrpc: '2.0',
    method: 'starknet_call',
