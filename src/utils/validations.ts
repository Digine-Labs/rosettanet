...
export function validateEthAddress(ethAddress: string): boolean {
  ...
  let address: string = ethAddress?.toLowerCase()?.startsWith("0x") ? ethAddress : `0x${ethAddress}`
...
export function validateSnAddress(snAddress: string): boolean {
  ...
  let address: string = addHexPrefix(removeHexPrefix(snAddress).padStart(64,"0"))
