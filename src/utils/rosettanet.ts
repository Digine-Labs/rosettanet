...
export async function getRosettaAccountAddress(ethAddress: string) : Promise<string> {
    ...
    const response: RPCResponse | string = await callStarknet({ // todo handle error if string
        jsonrpc: '2.0',
...
export async function isRosettaAccountDeployed(snAddress: string) : Promise<boolean> {
